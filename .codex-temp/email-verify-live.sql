...
DECLARE @token nvarchar(200) = N'1df0cda7a0004f1db6e6c51bad5a1f1213ad59b4c5754966997258ad43b7a621';
DECLARE @code nvarchar(20) = N'398265';
DECLARE @verificationLink nvarchar(max) = N'https://otelturizm.com/eposta-dogrula?email=irmhro0%40gmail.com&token=1df0cda7a0004f1db6e6c51bad5a1f1213ad59b4c5754966997258ad43b7a621&code=398265';
...
VALUES
(@userId, @email, @token, @code, 0, 0, 5, N'185.111.244.246', N'Codex manual verification test', DATEADD(HOUR, 24, SYSUTCDATETIME()), SYSUTCDATETIME());
...
VALUES
(@userId, @templateId, N'E-posta', @email, N'E-posta adresinizi doğrulayın', @body, @body, N'Beklemede', COALESCE((SELECT TOP (1) saglayici FROM email_services WHERE aktif_mi = 1 ORDER BY varsayilan_mi DESC, id ASC), N'SMTP'), N'users', @userId);
