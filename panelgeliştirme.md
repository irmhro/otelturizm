...
- [x] Admin master geçiş planı `Dokumanlar/admin-panel-gecis-plani.md` dosyasına route, cshtml, css, db ve e-posta canlı servis başlıklarıyla işlendi.
...
- [x] Admin komisyonlar sayfası `paneller/admin/commissions.css` standardına taşındı; ciro, tahsil edilen/bekleyen komisyon ve vergi kural formu Tabler düzenine alındı.
- [x] Admin partner başvuruları sayfası `paneller/admin/partner-applications.css` standardına taşındı; evrak, e-posta doğrulama ve admin giriş onayı tek kart akışında düzenlendi.
- [x] Admin rezervasyonlar tek liste sayfası `paneller/admin/unified-reservations.css` standardına taşındı; kaynak, ciro ve komisyon alanları eklendi.
- [x] Platform checkup ve eksiksiz gelişim raporu `Dokumanlar/platform-checkup-gelisim-raporu.md` dosyasına işlendi.
- [x] Admin `platform-checkup` sayfası eklendi; yetki, onay, satış, otel, oda, fiyat, görsel, e-posta, log ve konum tabloları canlı şemadan izleniyor.
- [x] Admin/firma/partner/satış eksik gelişim planı `Dokumanlar/admin-firma-partner-satis-eksik-gelisim-plani.md` dosyasına işlendi.
- [x] Admin `onay-merkezi` sayfası eklendi; partner/firma/otel/fatura/komisyon ana akışı tablo mantığında tek merkezde izleniyor.
- [ ] Admin roller-yetkiler ve konumlar sayfaları eklenecek; il/ilçe/mahalle ve `yetkiler/rol_yetkileri` kontrolleri admin panelden yapılacak.
...
- [x] Kullanıcı, partner, firma, satış, muhasebe ve destek departman dashboard iskeleti tablo ve metrik standardıyla eklendi.
- [x] Departman shell/sidebar/header/footer ve mobil menü kendi `departman` CSS dosyalarına ayrıldı.
