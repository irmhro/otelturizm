...
public class DeneyimlerService : IDeneyimlerService
{
    public DeneyimlerPageViewModel GetPage()
    {
        return new DeneyimlerPageViewModel
        {
            ...
            [
                ...
                new() { Title = "Hafta Sonu Fırsatları", Tag = "Club özel", LinkUrl = "/hotel?etiket=hafta-sonu-firsatlari", ImageUrl = "https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&auto=format&fit=crop&q=80" },
                new() { Title = "Spa & Wellness Otelleri", Tag = "Wellness", LinkUrl = "/hotel?q=spa+wellness", ImageUrl = "https://images.unsplash.com/photo-1540555700478-4be289fbecef?w=600&auto=format&fit=crop&q=80" }
            ],
            Categories =
            [
    ...
    private static List<DeneyimSmartRouteViewModel> BuildSmartRoutes()
    {
        ...
        return tags.Select(t => new DeneyimSmartRouteViewModel
        {
            ...
            SearchQuery = t.query,
            ColorClass = t.color,
            LinkUrl = $"/hotel?q={Uri.EscapeDataString(t.query)}"
        }).ToList();
    }
}
