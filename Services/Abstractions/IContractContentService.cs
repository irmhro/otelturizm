...
public interface IContractContentService
{
    ...
    Task<string?> GetPublicContractPdfUrlBySlugAsync(string slug, CancellationToken cancellationToken = default);
    ...
    Task QueueRegistrationContractBundleAsync(SqlConnection connection, SqlTransaction? transaction, long userId, string email, string? ipAddress, string? userAgent, CancellationToken cancellationToken = default);
