...
public sealed class PaymentCardCryptoService
{
    ...
    public PaymentCardCryptoService(IConfiguration configuration)
    {
        var keyBase64 = configuration["PaymentCards:EncryptionKeyBase64"];
        if (string.IsNullOrWhiteSpace(keyBase64))
        {
            throw new InvalidOperationException("PaymentCards:EncryptionKeyBase64 tanimli degil.");
        }

        _key = Convert.FromBase64String(keyBase64.Trim());
        if (_key.Length != 32)
        {
            throw new InvalidOperationException("PaymentCards:EncryptionKeyBase64 32 bayt (AES-256) olmalidir.");
        }
    }
