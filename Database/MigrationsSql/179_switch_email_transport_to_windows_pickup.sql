SET NOCOUNT ON;

DECLARE @metadata NVARCHAR(MAX) = N'{
  "username": "info@otelturizm.com",
  "imap_host": "mail.otelturizm.com",
  "imap_port": 993,
  "pop3_host": "mail.otelturizm.com",
  "pop3_port": 995,
  "smtp_host": "mail.otelturizm.com",
  "smtp_port": 465,
  "pickup_directory": "C:\\inetpub\\mailroot\\Pickup",
  "transport_mode": "pickup"
}';

UPDATE email_services
SET aktif_mi = 1,
    varsayilan_mi = 1,
    test_modu = 0,
    baglanti_zaman_asimi_saniye = CASE
        WHEN baglanti_zaman_asimi_saniye IS NULL OR baglanti_zaman_asimi_saniye < 30 THEN 60
        ELSE baglanti_zaman_asimi_saniye
    END,
    metadata = @metadata,
    guncellenme_tarihi = SYSUTCDATETIME()
WHERE servis_kodu = N'default_smtp';

UPDATE bildirim_loglari
SET durum = N'Beklemede',
    gonderme_denemesi = 1,
    hata_kodu = NULL,
    hata_mesaji = NULL
WHERE tur = N'E-posta'
  AND durum IN (N'Beklemede', N'Başarısız')
  AND gonderim_tarihi IS NULL;

IF OBJECT_ID(N'schema_migrations', N'U') IS NOT NULL
AND NOT EXISTS (SELECT 1 FROM schema_migrations WHERE script_name = N'179_switch_email_transport_to_windows_pickup.sql')
BEGIN
    INSERT INTO schema_migrations (script_name, checksum, applied_at)
    VALUES (N'179_switch_email_transport_to_windows_pickup.sql', N'manual-update', SYSUTCDATETIME());
END
