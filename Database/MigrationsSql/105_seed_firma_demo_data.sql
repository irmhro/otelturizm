...
DECLARE @firma_id BIGINT = (SELECT TOP (1) id FROM firmalar WHERE vergi_no = '1234567890');
...
SELECT 'Ahmet Yılmaz', 'ahmet.yilmaz@abcteknoloji.com', '05320000001', CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', '1585'), 2), 'firma_admin', @firma_id, 'Seyahat Operasyonları', 'Firma Seyahat Yöneticisi', 15000.00, 0, 'ABC-001', 1, 1, 'tr', 'TRY', 'Türkiye', SYSDATETIME()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE eposta = 'ahmet.yilmaz@abcteknoloji.com');

INSERT INTO users
(ad_soyad, eposta, telefon, sifre, rol, firma_id, departman, gorev_unvani, harcama_limiti, onay_gereksinimi, personel_kodu, firma_yonetici_mi, hesap_durumu, dil_tercihi, para_birimi, ulke, olusturulma_tarihi)
SELECT 'Şeyma Kaya', 'seyma.kaya@abcteknoloji.com', '05320000002', CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', '1585'), 2), 'firma_manager', @firma_id, 'Finans', 'Bütçe Yöneticisi', 9500.00, 1, 'ABC-002', 0, 1, 'tr', 'TRY', 'Türkiye', SYSDATETIME()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE eposta = 'seyma.kaya@abcteknoloji.com');

INSERT INTO users
(ad_soyad, eposta, telefon, sifre, rol, firma_id, departman, gorev_unvani, harcama_limiti, onay_gereksinimi, personel_kodu, firma_yonetici_mi, hesap_durumu, dil_tercihi, para_birimi, ulke, olusturulma_tarihi)
SELECT 'Emre Demir', 'emre.demir@abcteknoloji.com', '05320000003', CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', '1585'), 2), 'firma_staff', @firma_id, 'Satış', 'Bölge Satış Uzmanı', 6000.00, 1, 'ABC-003', 0, 1, 'tr', 'TRY', 'Türkiye', SYSDATETIME()
...
SELECT @firma_id, 'Finans', (SELECT TOP (1) id FROM users WHERE eposta = 'seyma.kaya@abcteknoloji.com'), 4500.00, 18000.00, 120000.00, 1, 7500.00, 1
WHERE NOT EXISTS (
  SELECT 1 FROM firma_harcama_limitleri WHERE firma_id = @firma_id AND departman = 'Finans'
);

INSERT INTO firma_harcama_limitleri (firma_id, departman, kullanici_id, gecelik_limit, rezervasyon_basi_limit, aylik_limit, onay_gereksinimi, otomatik_onay_limit, aktif_mi)
SELECT @firma_id, 'Satış', (SELECT TOP (1) id FROM users WHERE eposta = 'emre.demir@abcteknoloji.com'), 3500.00, 12000.00, 80000.00, 1, 5000.00, 1
WHERE NOT EXISTS (
...
SELECT @firma_id, ot.id, od.id, 5, 15, 20.00, ROUND(od.standart_gecelik_fiyat * 0.80, 2), CAST(GETDATE() AS DATE), DATEADD(DAY, 180, CAST(GETDATE() AS DATE)), 1, 10, 'Demo firma indirimi'
FROM oteller ot
