-- Idempotent (T308): 10 Istanbul ilce demo oteli — orkestra-bogaz tarzi, ork-demo-partner
-- Oncesi: 20260522_seed_ulkeler/iller/ilceler. Esdeger: 20260523_seed_istanbul_10_ilce_oteller.sql
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
BEGIN
    ...
    BEGIN
        INSERT INTO [dbo].[OTELLER](
            ...
            [TELEFON_1],[EPOSTA],[CHECK_IN_SAATI],[CHECK_OUT_SAATI],[TOPLAM_ODA_SAYISI],[KISA_ACIKLAMA],[UZUN_ACIKLAMA],
            [VARSAYILAN_KOMISYON_ORANI],[ODEME_VADESI],[ODEME_YONTEMI],[ORTALAMA_PUAN],[TOPLAM_YORUM_SAYISI],
            [YAYIN_DURUMU],[ONAY_DURUMU],[ONAY_TARIHI],[ONE_CIKAN_OTEL],[TAVSIYE_EDILEN_OTEL],[OLUSTURULMA_TARIHI])
        VALUES(
            ...
            N'2125550000',CONCAT(N'rez.',LOWER(@OtelKodu),N'@demo.otelturizm.local'),
            '14:00','12:00',20,N'Orkestra demo — Istanbul ilce.',N'Orkestra demo — Istanbul ilce.',
            15,N'Ayın 15''i',N'Online',@Puan,120,
            N'Yayında',N'Onaylandı',SYSUTCDATETIME(),@OneCikan,@Tavsiye,SYSUTCDATETIME());
        SET @HotelId = SCOPE_IDENTITY();
    END
    ELSE
    BEGIN
        UPDATE [dbo].[OTELLER] SET
            [OTEL_ADI]=@OtelAdi,[ILCE]=@IlceAdi,[MAHALLE]=@Mahalle,[ILCE_ID]=@IlceId,[SEHIR_ID]=@IstanbulIlId,[ULKE_ID]=@TrUlkeId,
    ...
    DECLARE @OzellikKategoriId bigint = (SELECT TOP (1) [ID] FROM [dbo].[OTEL_OZELLIK_KATEGORILERI] WHERE [AKTIF_MI]=1 ORDER BY [SIRALAMA], [ID]);
    IF @Havuz = 1 AND @OzellikKategoriId IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [dbo].[OTEL_OZELLIKLERI] WHERE [OZELLIK_ADI]=N'Havuz')
        INSERT INTO [dbo].[OTEL_OZELLIKLERI]([KATEGORI_ID],[OZELLIK_ADI],[OZELLIK_IKON],[AKTIF_MI],[SIRALAMA])
        VALUES(@OzellikKategoriId,N'Havuz',N'fa-water-ladder',1,1);

    IF @Havuz = 1
    BEGIN
        DECLARE @OzellikId bigint = (SELECT TOP (1) [ID] FROM [dbo].[OTEL_OZELLIKLERI] WHERE [OZELLIK_ADI]=N'Havuz' ORDER BY [ID]);
        IF @OzellikId IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [dbo].[OTEL_OZELLIK_ILISKILERI] WHERE [OTEL_ID]=@HotelId AND [OZELLIK_ID]=@OzellikId)
            INSERT INTO [dbo].[OTEL_OZELLIK_ILISKILERI]([OTEL_ID],[OZELLIK_ID],[AKTIF_MI]) VALUES(@HotelId,@OzellikId,1);
    END

    SELECT @RoomTypeId = [ID] FROM [dbo].[ODA_TIPLERI] WHERE [OTEL_ID]=@HotelId AND [ODA_TIP_KODU]=N'STD-DEMO';
    IF @RoomTypeId IS NULL
    BEGIN
        INSERT INTO [dbo].[ODA_TIPLERI]([OTEL_ID],[ODA_TIP_KODU],[ODA_ADI],[MAKSIMUM_KISI_SAYISI],[STANDART_GECELIK_FIYAT],[TOPLAM_ODA_SAYISI],[AKTIF_MI],[SIRALAMA])
        VALUES(@HotelId,N'STD-DEMO',N'Standart',2,@Fiyat,10,1,1);
...
PRINT N'T308 Istanbul 10 demo otel seed tamam (ORK-SEED-001..010).';
