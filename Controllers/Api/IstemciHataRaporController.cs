...
public sealed class IstemciHataRaporController : ControllerBase
{
    private readonly ILogger<IstemciHataRaporController> _logger;

    public IstemciHataRaporController(ILogger<IstemciHataRaporController> logger)
    {
        _logger = logger;
    }

    [HttpPost("/diagnostics/client-error")]
    public IActionResult Ingest([FromBody] IstemciHataYuk? payload)
    {
        if (payload is null)
        {
        ...
        var safe = IstemciHataYuk.Normalize(payload);
...
public sealed class IstemciHataYuk
{
    ...
    internal static IstemciHataYuk Normalize(IstemciHataYuk raw)
    {
